...
class FactualStrategy(BaseStrategy):
    ...
    async def extract(self, chunk: ProcessedChunk, llm_func=None) -> ProcessedChunk:
        ...
        prompt = f"""You are a factual knowledge extraction engine specialized in game chat.

Extract factual triples and entities from the text that are useful for answering game-related questions.

Priority extractions:
- Equipment/Item stats and effects (e.g., "银套" → 免疫 → "寄巢之唤")
- Version/Mod changes (e.g., "德雷" → 削弱了 → "银套病毒免疫")
- Game mechanics and configurations (e.g., "ftb.editing_mode" → 设为 → "false")
- Crafting/Progression info (e.g., "高级附魔台" → 需要 → "七咒模组")

Skip low-value triples:
- Chat meta (who said what, when)
- Social interactions, opinions without factual content
- Questions without answers

Language constraints:
- {language_guard}
- Preserve original game names, mod names, and configuration keys exactly.
