...
为 GameKnowledge WebUI 提供导入任务队列、状态管理、并发调度与取消/重试能力。
"""
...
logger = get_logger("GameKnowledge.WebImportManager")
...
class ImportTaskManager:
    ...
    def _default_maibot_source_db(self) -> Path:
        # game_knowledge/core/utils -> workspace root
        ...
        for attempt in range(retries + 1):
            try:
                result = await generate_with_resolved_model(
                    resolved_model,
                    request_type="GameKnowledge.WebImport",
                    prompt=prompt,
