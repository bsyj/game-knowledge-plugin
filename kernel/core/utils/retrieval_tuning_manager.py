...
logger = get_logger("GameKnowledge.RetrievalTuningManager")
...
class RetrievalTuningManager:
    ...
    async def _generate_nl_queries_with_llm(self, anchors: List[Dict[str, Any]], *, enabled: bool) -> Dict[str, str]:
        ...
        try:
            raw = await self._llm_call_text(prompt, request_type="GameKnowledge.RetrievalTuning.NLCaseGen")
    ...
    async def _suggest_profiles_with_llm(
        ...
        *,
        ...
        try:
            raw = await self._llm_call_text(prompt, request_type="GameKnowledge.RetrievalTuning.ProfileSuggest")
            obj = _safe_json_loads(raw)
            if not isinstance(obj, dict):
                return []
