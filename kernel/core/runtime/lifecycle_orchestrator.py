...
logger = get_logger("GameKnowledge.LifecycleOrchestrator")
...
async def ensure_initialized(plugin: Any) -> None:
    ...
    async with plugin._init_lock:
        ...
        logger.info("GameKnowledge 插件正在异步初始化存储组件...")
        ...
        if not bool(report.get("ok", False)):
            logger.error(
                "GameKnowledge runtime self-check failed: "
                f"{report.get('message', 'unknown')}; "
                "建议执行 python src/game_knowledge/scripts/runtime_self_check.py --json"
            )
        ...
        logger.info("GameKnowledge 插件异步初始化成功")
...
def start_background_tasks(plugin: Any) -> None:
    ...
    if (
        ...
        and bool(plugin.get_config("episode.enabled", False))
        and bool(plugin.get_config("episode.generation_enabled", False))
...
async def initialize_storage_async(plugin: Any) -> None:
    ...
    logger.info(f"GameKnowledge 数据存储路径: {data_dir}")
