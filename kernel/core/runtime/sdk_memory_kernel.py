...
from gk_shims.config_shim import global_config
...
logger = get_logger("GameKnowledge.SDKMemoryKernel")


def _game_knowledge_integration_config() -> Any:
    """Return GameKnowledge integration config, with legacy fallback during bootstrap."""
    game_cfg = getattr(global_config, "game_knowledge", None)
    if game_cfg is not None and hasattr(game_cfg, "integration"):
        return game_cfg.integration
    return object()
...
class SDKMemoryKernel:
    ...
    async def _refresh_runtime_self_check(self, *, sample_text: str = "GameKnowledge runtime self check") -> Dict[str, Any]:
    ...
    async def _recover_embedding_once(self, *, sample_text: str = "GameKnowledge runtime self check") -> Dict[str, Any]:
    ...
    async def initialize(self) -> None:
        ...
        report = await self._refresh_runtime_self_check(sample_text="GameKnowledge runtime self check")
    ...
    async def ingest_text(
        ...
        *,
        ...
        for chunk_index, chunk_text in enumerate(chunks):
            chunk_meta = dict(paragraph_meta)
            ...
            stored_hashes.append(paragraph_hash)
            if not primary_hash:
                primary_hash = paragraph_hash
        ...
        for name in entity_tokens:
            for paragraph_hash in stored_hashes:
        ...
        for row in [dict(item) for item in (relations or []) if isinstance(item, dict)]:
            ...
            self.metadata_store.link_paragraph_relation(primary_hash, result.hash_value)
            stored_relations.append(result.hash_value)

        self.metadata_store.upsert_external_memory_ref(
            external_id=external_token,
            paragraph_hash=primary_hash,
            source_type=source_type,
            metadata={"chat_id": chat_id, "person_ids": person_tokens, "chunk_count": len(chunks) if len(chunks) > 1 else 1},
        )
        for paragraph_hash in stored_hashes:
        ...
        payload = {"stored_ids": stored_hashes + stored_relations, "skipped_ids": []}
        if len(chunks) > 1:
    ...
    async def search_memory(self, request: KernelSearchRequest) -> Dict[str, Any]:
        ...
        if mode == "episode":
            if not bool(self._cfg("episode.enabled", False)):
                return {"summary": "", "hits": [], "error": "Episode 情景记忆检索已关闭"}
            ...
            hits = self._filter_query_relevant_hits(query, hits)
        ...
        if mode == "aggregate":
            payload = await self.aggregate_query_service.execute(
                ...
                episode_runner=(
                    (lambda: self._aggregate_episode(query, limit, request, time_window))
                    if bool(self._cfg("episode.enabled", False))
                    else None
                ),
            )
    ...
    async def _query_person_profile_with_feedback_refresh(
        ...
        *,
        ...
        _ = person_id, person_keyword, limit, force_refresh, source_note
        return {
            "success": False,
            "disabled": True,
            "reason": "game_knowledge_person_profile_removed",
            "summary": "",
            "traits": [],
            "evidence": [],
        }
    ...
    async def get_person_profile(self, *, person_id: str, chat_id: str = "", limit: int = 10) -> Dict[str, Any]:
        _ = chat_id
        return await self._query_person_profile_with_feedback_refresh(
            person_id=person_id,
    ...
    async def memory_graph_admin(self, *, action: str, **kwargs) -> Dict[str, Any]:
        ...
        if act == "get_graph":
            try:
                stats = self.metadata_store.get_statistics()
                has_metadata_graph = int(stats.get("entity_count", 0) or 0) > 0 or int(stats.get("relation_count", 0) or 0) > 0
                if has_metadata_graph and int(self.graph_store.num_nodes) == 0:
                    self._rebuild_graph_from_metadata()
                    self._persist()
            except Exception as exc:
                logger.warning(f"图谱懒重建失败，继续返回当前图谱: {exc}")
    ...
    async def memory_episode_admin(self, *, action: str, **kwargs) -> Dict[str, Any]:
        ...
        if act == "delete":
            ...
            if not episode_id:
                return {"success": False, "error": "episode_id 不能为空"}
            result = self.metadata_store.delete_episode(episode_id)
            if result.get("success"):
                result["note"] = "仅删除情景摘要，不删除底层知识或段落；后续重建可能再次生成相近情景。"
            return result
    ...
    async def memory_profile_admin(self, *, action: str, **kwargs) -> Dict[str, Any]:
        ...
        return {
            ...
            "disabled": True,
            "reason": "game_knowledge_person_profile_removed",
            "error": "GameKnowledge 不包含人物画像管理能力。",
        }
    ...
    async def memory_runtime_admin(self, *, action: str, **kwargs) -> Dict[str, Any]:
        ...
        if act in {"self_check", "refresh_self_check"}:
            report = await self._refresh_runtime_self_check(
                sample_text=str(kwargs.get("sample_text", "") or "GameKnowledge runtime self check")
            )
            checked_at = float(report.get("checked_at") or time.time())
    ...
    def _ensure_background_task(
        ...
        if task is not None and not task.done():
            return
        self._background_tasks[name] = asyncio.create_task(factory(), name=f"GameKnowledge.{name}")
    ...
    async def _episode_pending_loop(self) -> None:
        try:
            while not self._background_stopping:
                ...
                if not bool(self._cfg("episode.enabled", False)):
                    continue
                if not bool(self._cfg("episode.generation_enabled", False)):
                    continue
                await self.process_episode_pending_batch(
    ...
    def _resolve_feedback_related_person_ids(
        ...
        *,
        ...
        _ = old_relation_rows, corrected_relations
        return []
    ...
    def _feedback_cfg_enabled() -> bool:
        memory_cfg = _game_knowledge_integration_config()
    ...
    def _serialize_graph(self, *, limit: int = 200) -> Dict[str, Any]:
        ...
        for name in nodes:
            ...
            entity_row = self.metadata_store.query(
                "SELECT appearance_count FROM entities WHERE name = ? AND (is_deleted IS NULL OR is_deleted = 0) LIMIT 1",
                (name,),
            )
            count = int(entity_row[0].get("appearance_count", 0) or 0) if entity_row else 0
            node_payload.append({"id": name, "name": name, "count": count, "attributes": attrs})
        ...
        for source, target, relation_hashes in self.graph_store.iter_edge_hash_entries():
            ...
            primary_hash = relation_hash_tokens[0] if relation_hash_tokens else ""
            edge_payload.append(
                {
                    "source": source,
                    "target": target,
                    ...
                    "confidence": float(self.graph_store.get_edge_weight(source, target)),
                    "hash": primary_hash,
    ...
    def _retrieval_result_hit(item: RetrievalResult) -> Dict[str, Any]:
        ...
        return {
            ...
            "source": meta.get("source_doc") or payload.get("source", ""),
            "metadata": meta,
        }
    ...
    @classmethod
    def _query_anchor_phrases(cls, query: str) -> List[str]:
        text = str(query or "").strip().lower()
        if not text:
            return []
        separators = cls._QUERY_STOP_TERMS | cls._BROAD_INTENT_TERMS | {"配", "用", "有", "无"}
        pattern = "|".join(re.escape(term) for term in sorted(separators, key=len, reverse=True))
        reduced = re.sub(pattern, " ", text)
        phrases: List[str] = []
        phrases.extend(re.findall(r"[a-z0-9_]{2,}", reduced))
        phrases.extend(re.findall(r"[\u4e00-\u9fff]{2,}", reduced))
        phrases.extend(re.findall(r"(?<![\u4e00-\u9fff])[\u4e00-\u9fff](?![\u4e00-\u9fff])", reduced))

        out: List[str] = []
        seen: set[str] = set()
        for phrase in phrases:
            token = str(phrase or "").strip().lower()
            if len(token) < 1 or token in cls._QUERY_STOP_TERMS or token in cls._BROAD_INTENT_TERMS or token in seen:
                continue
            seen.add(token)
            out.append(token)
        return out
    ...
    def _query_terms(cls, query: str) -> List[str]:
        ...
        try:
            ...
            jieba_terms = [str(token).strip().lower() for token in jieba.lcut(text)]
        ...
        meaningful_jieba = [
            token
            for token in jieba_terms
            if len(token) >= 2 and token not in cls._QUERY_STOP_TERMS
        ]
        if not meaningful_jieba:
        ...
        for raw in terms:
            ...
            is_single_cjk = bool(re.fullmatch(r"[\u4e00-\u9fff]", token))
            if (len(token) < 2 and not is_single_cjk) or token in cls._QUERY_STOP_TERMS or token in seen:
                continue
            seen.add(token)
            unique.append(token)
        return unique
    ...
    @classmethod
    def _filter_query_relevant_hits(cls, query: str, hits: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Drop hits that only match broad intent words such as "用/获取/攻击".

        The retriever can surface relation triples where the only overlap is a
        generic predicate. For a knowledge tool it is better to return fewer
        hits than to feed unrelated text to the model.
        """
        if not hits:
            return hits
        ...
        phrase_terms = cls._query_anchor_phrases(query)
        specific_terms = phrase_terms + [
            term
            for term in terms
            if term not in cls._BROAD_INTENT_TERMS and not term.isdigit()
        ]
        specific_terms = list(dict.fromkeys(specific_terms))
        anchor_terms = specific_terms or terms
        if not anchor_terms:
            anchor_terms = terms
        required_anchor_matches = 2 if len(specific_terms) >= 2 else 1
        expanded_anchor_terms: List[str] = []
        for term in anchor_terms:
            expanded_anchor_terms.extend(cls._QUERY_SYNONYMS.get(term, (term,)))
        anchor_terms = list(dict.fromkeys(expanded_anchor_terms))
        ...
        for item in hits:
            ...
            try:
                ...
                score = 0.0
            if item_type == "episode" and score <= 0.0:
                continue
            ...
            if not content:
                continue
            matched_count = sum(1 for term in anchor_terms if term in content)
            if matched_count >= required_anchor_matches:
                filtered.append(item)

        return filtered

    def _anchor_fts_fallback(self, query: str, limit: int) -> List[Dict[str, Any]]:
        if self.metadata_store is None:
            return []

        phrase_anchors = self._query_anchor_phrases(query)
        term_anchors = [
            term
            for term in self._query_terms(query)
            if term not in self._BROAD_INTENT_TERMS and term not in self._QUERY_STOP_TERMS and not term.isdigit()
        ]
        anchors = list(dict.fromkeys(phrase_anchors + term_anchors))
        if not anchors:
            return []
        metadata_hits = self._metadata_question_fallback(query, anchors, limit)
        like_hits = self._merge_preferred_hits(
            metadata_hits,
            self._anchor_like_fallback(query, anchors, limit),
            limit,
        )

        candidates: List[List[str]] = []
        if len(phrase_anchors) >= 2:
            candidates.append(phrase_anchors[:2])
        if phrase_anchors:
            candidates.append(phrase_anchors[:1])
        if len(term_anchors) >= 2:
            candidates.append(term_anchors[:3])
            candidates.append(term_anchors[:2])
        if term_anchors:
            candidates.append(term_anchors[:1])

        seen_queries: set[str] = set()
        for group in candidates:
            safe_terms = []
            for term in group:
                token = str(term or "").replace('"', '""').strip()
                if token:
                    safe_terms.append(f'"{token}"')
            if not safe_terms:
                continue
            match_query = " ".join(safe_terms)
            if match_query in seen_queries:
                continue
            seen_queries.add(match_query)
            ...
            hits = [
                {
                    ...
                    "content": str(row.get("content", "") or ""),
                    "score": max(0.0, -float(row.get("bm25_score", 0.0) or 0.0)),
                    "type": "paragraph",
                    "source": str(row.get("source", "") or ""),
                    "metadata": {},
                }
            ...
            filtered = self._filter_query_relevant_hits(query, hits)
            if filtered:
    ...
    def _decode_row_metadata(raw: Any) -> Dict[str, Any]:
        ...
        if isinstance(raw, (bytes, bytearray, memoryview)):
            try:
                decoded = pickle.loads(bytes(raw))
    ...
    def _anchor_like_fallback(self, query: str, anchors: List[str], limit: int) -> List[Dict[str, Any]]:
        if self.metadata_store is None or not anchors:
            return []

        like_terms = [term for term in dict.fromkeys(anchors) if str(term or "").strip()]
        if not like_terms:
            return []

        conditions = " OR ".join(["content LIKE ? OR metadata LIKE ?"] * len(like_terms[:8]))
        params: List[Any] = []
        for term in like_terms[:8]:
            pattern = f"%{term}%"
            params.extend([pattern, pattern])
        params.append(max(5000, int(limit) * 400))
        rows = self.metadata_store.query(
            f"""
            SELECT hash, content, metadata
            FROM paragraphs
            WHERE (is_deleted IS NULL OR is_deleted = 0)
              AND ({conditions})
            LIMIT ?
        ...
        for row in rows:
            content = str(row.get("content", "") or "")
            metadata = self._decode_row_metadata(row.get("metadata"))
            metadata_text = " ".join(
                str(value)
                for key, value in metadata.items()
                if key
            ...
            if not matched:
                continue
            question = str(metadata.get("question", "") or "").lower()
            title = str(metadata.get("title", "") or "").lower()
            normalized_question = self._normalize_query_text(question)
            question_hits = sum(1 for term in matched if str(term).lower() in question)
            title_hits = sum(1 for term in matched if str(term).lower() in title)
            exact_question_bonus = 0.0
            if normalized_query and normalized_question:
                if normalized_query == normalized_question:
                    exact_question_bonus = 20.0
                elif normalized_query in normalized_question or normalized_question in normalized_query:
                    exact_question_bonus = 8.0
            score = (
                float(len(matched))
                + question_hits * 2.0
                + title_hits * 1.0
                + exact_question_bonus
                + (sum(len(str(term)) for term in matched) / max(1, len(haystack)))
            )
            scored.append(
                {
                    "hash": str(row.get("hash", "") or ""),
                    ...
                    "score": score,
                    "type": "paragraph",
                    "source": "anchor_like_fallback",
                    "metadata": metadata,
                }
            )
        scored.sort(key=lambda item: float(item.get("score", 0.0) or 0.0), reverse=True)
        return self._filter_query_relevant_hits(query, scored[: max(1, int(limit))])

    def _metadata_question_fallback(self, query: str, anchors: List[str], limit: int) -> List[Dict[str, Any]]:
        if self.metadata_store is None or not anchors:
            return []

        like_terms = [str(term or "").strip().lower() for term in dict.fromkeys(anchors) if str(term or "").strip()]
        if not like_terms:
            return []
        ...
        for row in rows:
            ...
            if not metadata_text:
                continue
            matched = [term for term in like_terms if term in metadata_text]
            normalized_question = self._normalize_query_text(question)
            exact_question_bonus = 0.0
            if normalized_query and normalized_question:
                if normalized_query == normalized_question:
                    exact_question_bonus = 30.0
                elif normalized_query in normalized_question or normalized_question in normalized_query:
                    exact_question_bonus = 12.0
            if not matched and exact_question_bonus <= 0.0:
                continue
            score = float(len(matched)) + exact_question_bonus + sum(len(term) for term in matched) / max(1, len(metadata_text))
            scored.append(
                {
                    "hash": str(row.get("hash", "") or ""),
                    "content": str(row.get("content", "") or ""),
                    "score": score,
                    "type": "paragraph",
                    "source": "metadata_question_fallback",
                    "metadata": metadata,
                }
            )
    ...
    def _merge_preferred_hits(
        ...
        for item in list(preferred) + list(existing):
            ...
            if not key:
                key = str(item.get("content", "") or "")[:160]
            if key in seen:
                continue
            seen.add(key)
            merged.append(item)
