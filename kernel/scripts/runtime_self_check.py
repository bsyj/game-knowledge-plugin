#!/usr/bin/env python3
"""Run GameKnowledge runtime self-check against real embedding/runtime configuration."""
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="GameKnowledge runtime self-check")
    parser.add_argument(
        "--config",
        default=str(DEFAULT_CONFIG_PATH),
        help="config.toml path (default: config/game_knowledge.toml)",
    )
    parser.add_argument(
        "--data-dir",
        default="",
    ...
    parser.add_argument(
        "--sample-text",
        default="GameKnowledge runtime self check",
...
async def _main_async(args: argparse.Namespace) -> int:
    ...
    if args.data_dir:
        ...
        temp_dir_ctx = tempfile.TemporaryDirectory(prefix="game-knowledge-runtime-self-check-")
    ...
    try:
        ...
        report = await run_embedding_runtime_self_check(
            ...
            sample_text=str(args.sample_text or "GameKnowledge runtime self check"),
        )
        ...
        if args.json:
            ...
            print("GameKnowledge Runtime Self-Check")
